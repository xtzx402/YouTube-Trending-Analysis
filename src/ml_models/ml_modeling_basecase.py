...
def run_baseline_model(output_file: str = "outputs/ml_test_predictions_baseline.csv",
                       model_name: str = "baseline_no_emotion"):
    """Train and evaluate baseline logistic regression model without emotion features.

    Reads cleaned video data, preprocesses numeric and categorical features,
    trains a logistic regression model with calibration, evaluates performance,
    and exports predictions and metrics.

    Args:
        output_file (str): Path to save prediction CSV.
        model_name (str): Identifier to store metrics in the database.
    ...
    try:
        ...
        query = """
        SELECT 
            video_id, title, video_category, likes, dislikes,
            comment_count, views, publish_hour, title_len,
            tags_count, days_on_trending, long_term_trending
        FROM us_videos;
        """
        df = pd.read_sql(query, con=engine)
        logging.info(f"Loaded {len(df)} rows from us_videos (cleaned).")
    ...
    # Export predictions
    pred_df = df.loc[idx_test, ["video_id", "title", "video_category",
